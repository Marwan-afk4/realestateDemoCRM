...
@endphp
@section('title', __('Create Unit Sub-Type'))
@section('content')
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <h1 class="mb-3">{{__('Create Unit Sub-Type')}}</h1>
            <div class="main-card mb-3 card">
                <div class="card-body">
                    <form action="{{ route('unit-sub-types.store') }}" method="POST">
                        @csrf
                        <div class="form-group mb-3">
                            ...
                            <select name="uptown_type_id" id="uptown_type_id" class="form-control @error('uptown_type_id') is-invalid @enderror" required>
                                ...
                                @foreach($uptownTypes as $type)
                                    <option value="{{ $type->id }}" {{ old('uptown_type_id') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
