@extends('layouts.app')
@section('title', __('Edit Administrative Data Field'))
@section('content')
<div class="container-fluid">
    <h1 class="mb-3">{{ __('Edit Administrative Data Field') }}</h1>
    <div class='main-card mb-3 card'>
        <div class='card-body'>
            <form action="{{ route('administrative-data.update', $field) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                ...
                <a href="{{ route('administrative-data.index') }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
            </form>
        </div>
    </div>
</div>
